...
is_big_endianess = False


def toBigEndianessVal(value):
    from math import log
    if value == 0:
        bytes_number = 1;
    else:
        bytes_number = int(log(value, 256)) + 1

    #only support unsigned short, unsigned int and unsigned long
    if bytes_number == 2:
        format_str = '>H'
    elif bytes_number == 4:
        format_str = '>I'
        ...
        raise Invalid("Unsupported format value")
        
    val = int("0x%x" % struct.unpack(format_str, (value).to_bytes(bytes_number, 'little')), 16)
...
class FileHeader2173(Structure):
    ...
    SYSSTAT_MAGIC value:
        ...
        0x2173 (11.6.x, RHEL 7-Beta1, Fedora 19+)
        0x2175 (12.0.x, RHEL 7-Beta1, Fedora 19+)
...
def get_file_activity_2171(fp, fh):
    ...
    file_activities = []
    total_len = 0
    if is_big_endianess is True:
        fh.sa_nr_act = toBigEndianessVal(fh.sa_nr_act)
    for i in range(fh.sa_nr_act):
...
class_map = {
    ...
    0x2173: {
        ...
        "file_activity": get_file_activity_2171,
        "os-code": "f20",
        "rpm-versions": ("sysstat-11.6.x-1.el8",),
        },
    0x2175: {
        "file_magic": FileMagic,
        "file_header": FileHeader2173,
        ...
        "file_activity": get_file_activity_2171,
        "os-code": "f21",
        "rpm-versions": ("sysstat-12.0.x-1.el9",),
        },
    }


def fetch_fileheader_with_fp(fp):
    global is_big_endianess
    fm = FileMagic()
    ret = fp.readinto(fm)
    ...
    if fm.sysstat_magic == SYSSTAT_MAGIC:
        ...
        little_sysstat_magic = toBigEndianessVal(fm.sysstat_magic)
        if little_sysstat_magic == SYSSTAT_MAGIC:
            is_big_endianess = True
            
    if fm.sysstat_magic == SYSSTAT_MAGIC or little_sysstat_magic == SYSSTAT_MAGIC:
        if is_big_endianess is True:
            fm.format_magic = toBigEndianessVal(fm.format_magic)
    ...
    the_class_map['os-code'] = "{0}.{1}.{2}".format(fm.sysstat_version, fm.sysstat_patchlevel, fm.sysstat_sublevel)
